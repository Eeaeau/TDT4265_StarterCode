...
def test_get_all_box_matches():
    ...
    b2 = np.array([[0, 0, 1, 1], [0.25, 0.25, 1, 1]])
    res1, res2 = get_all_box_matches(b1, b2, 1)
    assert np.all(res1 == b1)
    assert np.all(res2 == b2[0:1])

    b2 = np.array([[0.25, 0.25, 1, 1], [0, 0, 1, 1]])
    res1, res2 = get_all_box_matches(b1, b2, 1)
    assert np.all(res1 == b1)
...
def test_calculate_individual_image_result():
    ...
    b1 = np.array([[0, 0, 1, 1], [0.5, 0.5, 1.5, 1.5], [2, 2, 3, 3], [5.5, 5.5, 8, 8]])
    b2 = np.array([[0, 0, 1, 1], [0, 0, 1.5, 1.5], [3, 3, 4, 4], [5, 5, 8, 8]])
    ...
    assert res["true_pos"] == ans1, "Expected {}, got: {}".format(ans1, res["true_pos"])
