...
class Resnet101WithFPN(torch.nn.Module):
    def __init__(self,
        ...
        self.output_feature_shape = output_feature_sizes
        m = torch.hub.load('pytorch/vision:v0.11.2', model_type, pretrained=pretrained)
        ...
        self.body = torchvision.models.feature_extraction.create_feature_extractor(
            m, return_nodes={f'layer{k}': str(v)
                             for v, k in enumerate([1, 2, 3, 4,4,4])})
        ...
        # Build FPN
        self.out_channels = [128, 128, 128, 128, 128, 128]
        self.num_out_channels = len(self.out_channels)
        self.fpn = torchvision.ops.FeaturePyramidNetwork(
            in_channels_list, out_channels=128)
